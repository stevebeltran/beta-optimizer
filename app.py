  ...
  if(el){el.style.transition='opacity 0.35s ease';el.style.opacity='0';
    ...
    if st.session_state['csvs_ready']:
            ...
            for d in active_drones:
                ...
                if d.get('pinned'):
                    fig.add_trace(go.Scattermap(
                        ...
                        marker=dict(size=32, color=d['color'], symbol='star'),
            ...
            if not custom_station_df.empty:
                ...
                if not _inactive_custom.empty:
                    ...
                    fig.add_trace(go.Scattermap(
                        ...
                        lon=_custom_lon,
                        mode='markers',
                        marker=dict(size=18, color=_custom_color, symbol='diamond'),
            ...
            if _suggestions and show_station_suggestions and st.session_state.get('show_suggestion_markers', True):
                ...
                if _sug_off_lat:
                    fig.add_trace(go.Scattermap(
                        ...
                        textfont=dict(size=12, color='#8B6F47', weight=700),
                        marker=dict(size=10, color='rgba(232, 178, 153, 0.8)'),
        ...
        try:
            ...
            _qr_report_id = _tracked_report_id or str(st.session_state.get("public_report_id", "")).strip()
            _qr_url = ""
            if _qr_report_id:
                try:
                    _qr_url = _build_public_report_url(_qr_report_id)
