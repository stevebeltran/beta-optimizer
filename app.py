...
def fetch_faa_airspace_grids(minx, miny, maxx, maxy):
    ...
    # Step 1: Identify all airspaces intersecting our map view
    q1_url = (
    ...
    try:
        ...
        with urllib.request.urlopen(req1, timeout=10) as resp1:
            ...
            # Find which exact field stores the airport identifier dynamically
            for feat in data1.get('features', []):
                attrs = feat.get('attributes', {})
                if not field_name:
                    if 'APT1_FAAID' in attrs: field_name = 'APT1_FAAID'
                    elif 'APT1_NAME' in attrs: field_name = 'APT1_NAME'
                    elif 'AIRSPACE' in attrs: field_name = 'AIRSPACE'
                    elif 'APT1_ICAO' in attrs: field_name = 'APT1_ICAO'
                
                if field_name and attrs.get(field_name):
                    airspaces.add(attrs[field_name])
                    
            if not airspaces or not field_name:
                return None
                
            # Step 2: Fetch full grids ONLY for those identified airspaces so grids don't clip at the screen edges
            in_clause = ",".join(["'" + str(a).replace("'", "''") + "'" for a in airspaces])
            where_clause = urllib.parse.quote(f"{field_name} IN ({in_clause})")
            
            q2_url = f"{base_url}?f=geojson&where={where_clause}&outFields=CEILING,{field_name}&outSR=4326"
            
            req2 = urllib.request.Request(q2_url, headers={'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req2, timeout=15) as resp2:
                data2 = json.loads(resp2.read().decode('utf-8'))
                
                # Filter out null ceilings
                if data2 and 'features' in data2:
                    valid_features = [f for f in data2['features'] if f.get('properties', {}).get('CEILING') is not None]
                    data2['features'] = valid_features
                    if valid_features:
                        return data2
    except Exception as e:
...
if st.session_state['csvs_ready']:
    ...
    with ranges_placeholder:
        resp_radius_mi = st.slider("🚁 Responder Range (Miles)", 2.0, 3.0, st.session_state.get('r_resp', 2.0), step=0.5, help="The effective operational flight radius for short-range Responder drones.")
        guard_radius_mi = st.slider("🦅 Guardian Range (Miles)", 1, 8, int(st.session_state.get('r_guard', 8)), help="The effective operational flight radius for long-range Guardian drones.")
    ...
    with counts_placeholder:
        ...
        k_guardian = st.slider("🦅 Guardian Count", 0, max_g, safe_guard_val, help="Number of long-range heavy-lift drones to deploy.")
    ...
    with map_col:
        ...
        if show_faa and faa_geojson and 'features' in faa_geojson:
            lats_400, lons_400 = [], []
            lats_restricted, lons_restricted = [], []
            ...
            for f in faa_geojson['features']:
                ...
                if geom and geom['type'] == 'Polygon':
                    ...
                    if ceil == 400:
                        lats_400.extend(lat_list + (None,))
                        lons_400.extend(lon_list + (None,))
                        text_color = "#39FF14" if is_dark else "#28a745"
                    else:
                        lats_restricted.extend(lat_list + (None,))
                        lons_restricted.extend(lon_list + (None,))
            ...
            if lats_restricted:
                fig.add_trace(go.Scattermapbox(
                    lat=lats_restricted, lon=lons_restricted, mode='lines',
                    line=dict(color="rgba(255, 50, 50, 0.85)", width=2),
                    name='Restricted (<400ft)', hoverinfo='skip'
                ))
            if lats_400:
                fig.add_trace(go.Scattermapbox(
                    lat=lats_400, lon=lons_400, mode='lines',
                    line=dict(color="rgba(57, 255, 20, 0.85)", width=1.5),
                    name='Permitted (400ft)', hoverinfo='skip'
                ))
            if text_lats:
                fig.add_trace(go.Scattermapbox(
