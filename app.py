...
def solve_mclp(resp_matrix, guard_matrix, num_resp, num_guard):
    ...
    # 1. Create separate decision variables for Responders and Guardians
    ...
    # 2. Strict limits for each specific drone type
    ...
    # 3. Only allow 1 drone total per station
    for s in range(n_stations):
        model += x_resp[s] + x_guard[s] <= 1

    # 4. Corrected coverage logic
    for call in range(n_calls):
        cover = []
        for s in range(n_stations):
            # Only append the specific variable if it mathematically reaches the call
            if resp_matrix[s][call]:
                cover.append(x_resp[s])
            if guard_matrix[s][call]:
                cover.append(x_guard[s])

        # If the call can be covered by anything, link it to the y variable
        if cover:
            model += y[call] <= pulp.lpSum(cover)
        else:
            # If no station can reach this call, force y to 0
            model += y[call] == 0

    model.solve(pulp.PULP_CBC_CMD(msg=0))

    # 5. Extract specific lists based on the separate variables
