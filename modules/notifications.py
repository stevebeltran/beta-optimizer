...
def _notify_crash_email(step, error_message, traceback_text, details=None):
    ...
    _log_crash_to_sheets(step, error_message, traceback_text, details, event_type="CRASH_REPORT")
    try:
        gmail_address = st.secrets.get("GMAIL_ADDRESS", "")
        app_password = st.secrets.get("GMAIL_APP_PASSWORD", "")
        notify_address = st.secrets.get("NOTIFY_EMAIL", gmail_address)
        ...
        if not gmail_address or not app_password or not recipients:
            reason = "missing GMAIL_ADDRESS secret" if not gmail_address else (
                "missing GMAIL_APP_PASSWORD secret" if not app_password
                else "no NOTIFY_EMAIL or NOTIFY_SMS_EMAIL recipients configured"
            )
        ...
        print(f"[BRINC] Crash email send failed: {detail}")
        _log_crash_to_sheets(
            step, error_message, traceback_text, details,
